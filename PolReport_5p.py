...
def main_calculations(bioprobe, base_dir, thermal_dir, hyperpol_dir, t):
    """
    ...
    bioprobe_info = {
        0: {'name': 'Urea', 'peak_region': [170, 160]},
        1: {'name': 'Pyruvate', 'peak_region': [174, 166]},
        2: {'name': 'C2-Pyruvate', 'peak_region': [207, 203]},
        3: {'name': 'Fumarate', 'peak_region': [178, 169]},
        4: {'name': 'HP001', 'peak_region': [26, 17]},
    ...
    I_hyp_ALL /= I_hyp_ALL[0]

    # Perform exponential decay fitting for T1 estimation
    first_points = [0,5,10,15,20]
    # Sometimes Spinsolve doesn't capture the second point for some reason
    if len(hyperpol_list) < 26:
        first_points = [0,4,9,14,19]
    
    # Exp fit INSIDE magnet
    popt_in = []
    for i in range(5):
        popt, _ = curve_fit(func, t_list[first_points[i]:first_points[i]+5], I_hyp_ALL[first_points[i]:first_points[i]+5],p0=[2,60])
        popt_in.append(popt)
    
    # Exp fit OUTSIDE magnet
    popt, pcov = curve_fit(func, t_list[first_points], I_hyp_ALL[first_points],p0=[2,60])
    
    # Go to script dir for temporary figure saving
    os.chdir(base_dir)
    
    # Plot the results (fitted curves and raw data)
    figure = plt.figure(figsize=[10,5])
    plt.plot(t_list,func(t_list,popt[0],popt[1]),'k', label = f'$T_1$ outside magnet: $I(t) = $${{e^{{-t/{popt[1]:.1f}}}}}$')
    for i in range(5):
        plt.plot(t_list[first_points[i]:first_points[i]+5],func(t_list[first_points[i]:first_points[i]+5],popt_in[i][0],popt_in[i][1]),'--',color = 'gray', label = f'$T_1$ inside magnet [{i+1}]: $I(t) = $${{e^{{-t/{popt_in[i][1]:.1f}}}}}$')
    plt.plot(t_list,I_hyp_ALL,'r.',markersize=7,label = 'Data points inside')
    plt.plot(t_list[first_points],I_hyp_ALL[first_points],'b.',markersize=10, label = f'Data points outside ($t_0 = {t:.1f}$ s)')
    plt.xlabel('Time (s)')
    plt.ylabel('Normalized integral (real)')
    plt.legend()
    ...
    T1 = popt[1]
    polarization_liquid = I_hyp / I_them / angle_correction * boltzmann_dist / dilution
    polarization_solid = polarization_liquid / np.exp(-t/T1)
    enhancement_factor = I_hyp / I_them / angle_correction
    
    return polarization_solid, polarization_liquid, enhancement_factor, bioprobe_name, popt

def generate_pdf(polarization_solid, polarization_liquid, enhancement_factor, bioprobe_name, popt, t, base_dir, thermal_dir, hyperpol_dir):
    """
    ...
    my_canvas = canvas.Canvas(f'{base_dir}/Reports/{hyperpol_dir_basename}_HPpolreport_5p.pdf')

    # Title and metadata
    add_text(my_canvas,f'5-point Hyperpolarization Report for Data ({bioprobe_name}):',[220,50,50],20,750)
    ...
    add_text(my_canvas,f'Polarization Liquid: {100*polarization_liquid:.1f} %',[0,0,0],15,630)
    add_text(my_canvas,f'Enhancement factor: {enhancement_factor:.1e}',[0,0,0],10,615)
    add_text(my_canvas,f'T1 outside magnet: {popt[1]:.1f} s',[0,0,0],15,585)
...
def main():
    ...
    polarization_solid, polarization_liquid, enhancement_factor, bioprobe_name, popt = main_calculations(bioprobe, base_dir, thermal_dir, hyperpol_dir, t)
    # Run function to generate PDF with the computed results.
    generate_pdf(polarization_solid, polarization_liquid, enhancement_factor, bioprobe_name, popt, t, base_dir, thermal_dir, hyperpol_dir)
